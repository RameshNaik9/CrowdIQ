...
#     # For local cameras, open device index 0
#     if rtsp_url.lower() == "local":
#         logger.info(
#             f"Local inference triggered for camera {camera_id}. Accessing local camera device."
#         )
...
#             raise HTTPException(
#                 400,
#                 "Failed to open local camera device. Please ensure the camera is not in use by another application.",
#             )
#         test_cap.release()

#         # Start the processing thread for local camera
#         stream_thread = threading.Thread(
#             target=process_local_stream, args=(camera_id, user_id), daemon=True
#         )
#         stream_thread.start()
...
#     # For RTSP cameras, validate the stream
#     test_cap = cv2.VideoCapture(rtsp_url)
#     if not test_cap.isOpened():
#         raise HTTPException(400, f"Failed to connect to RTSP URL: {rtsp_url}")
#     test_cap.release()
